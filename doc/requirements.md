...
Block Hop 是一个 `action` 类型的移动端小游戏：TikTok feed 上的等距 voxel 跳格过马路 — 点屏跳一步, 划屏换向, 黄出租和 F 线地铁会把你压成一片, 过河必须站木头, 镜头追上 = 完。复用 Shelf It 同款 Family-B 矢量体素角色, NYC 街头主题。距离排行榜。AlterU action 系列首发。

## 2. Visual Design

- 整体布局：页面占用 7 x 0 px，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #fff、#8a7d72、rgba(21,17,14,.55)、#8a8175、rgba(255,247,230,.92)、rgba(21,17,14,.6)、#fffdf6、#15110e；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 -apple-system,"PingFang SC",BlinkMacSystemFont,"Segoe UI",sans-serif、var(--comic)、var(--pow)，按钮与状态文字保持 12-24 px 的可读范围。
